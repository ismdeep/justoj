...
class Group extends ApiBaseController
{
	...
	public function join($group_id, $password = '')
	{
		...
		// 判断用户是否已经有加入此班级记录
		if (GroupJoinModel::get(['group_id' => $group_id, 'user_id' => $this->loginuser->user_id])) {
		...
		if (0 == $group->type) {
			...
			$group_join->user_id = $this->loginuser->user_id;
...
//		$group_join->save();
	...
	public function add_task($group_id, $title, $begin_time, $end_time, $description, $problem_ids)
	{
		...
		$patten = "/^\d{4}[\-](0?[1-9]|1[012])[\-](0?[1-9]|[12][0-9]|3[01])(\s+(0?[0-9]|[12][0-3])\:(0?[0-9]|[1-5][0-9])\:(0?[0-9]|[1-5][0-9]))?$/";
